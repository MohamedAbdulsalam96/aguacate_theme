...
(function() {
	...
	frappe.views.KanbanBoardCard = function(card, wrapper) {
		...
		function render_card_meta() {
			...
			const $assignees_action = get_assignees_action();
			//console.log($assignees_group[0].innerHTML);
			console.log(card);
			...
			self.$card.find(".kanban-card-meta").empty().append(html)
				.find('.kanban-assignments').append($assignees_group[0].innerHTML);
			self.$card.find('.kanban-assignments').append($assignees_action);
			$(".avatar-action").attr("title", __("Assign to")).tooltip({
		...
		function get_assignees_group() {
			console.log('users: ',card.assigned_list);
			return frappe.avatar_group(card.assigned_list, 3, {
				css_class: 'avatar avatar-small avatar-group'
			});
		}

		function get_assignees_action() {
			console.log('users: ',card.assigned_list);
			return frappe.avatar_group(card.assigned_list, 3, {
				css_class: 'avatar avatar-small avatar-group',
				action_icon: 'add',
